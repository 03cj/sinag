...
const DashboardC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let totalPartnerHTE = 10; // This can remain static if not filtered by program

      if (selectedProgram === 'All') {
        internsForProgram = dummyProgramChartData;
        activeProgramsCount = dummyProgramChartData.length; // Count all programs
      } else {
        internsForProgram = dummyProgramChartData.filter((item) => item.program === selectedProgram);
      ...
      setKpiData({
        ...
        partnerHTE: String(totalPartnerHTE),
      });
      ...
      setCompanyChartData({
        ...
        datasets: [
          {
            data: companyValues,
            backgroundColor: ['#FF0000', '#FFFF00', '#F5DEB3', '#C0C0C0', '#ADD8E6'],
            borderColor: ['white', 'white', 'white', 'white', 'white'],
            borderWidth: 1,
          },
        ],
      ...
      /*
            ...
            try {
                ...
                setKpiData({
                    ...
                    partnerHTE: data.totalPartnerHTE,
                });
        ...
        labels: {
          ...
          generateLabels: (chart) => {
            ...
            if (data.labels.length && data.datasets.length) {
              const currentChartPercentages =
                chart.canvas.id === 'program-pie-chart' ? programChartData?.percentages : companyChartData?.percentages;
      ...
      tooltip: {
        callbacks: {
          label: function (context) {
            ...
            if (
              chartId === 'program-pie-chart' &&
              programChartData &&
              programChartData.percentages &&
              context.dataIndex !== undefined
            ) {
              label += ` (${programChartData.percentages[context.dataIndex]}%)`;
            } else if (
              chartId === 'company-pie-chart' &&
              companyChartData &&
              companyChartData.percentages &&
              context.dataIndex !== undefined
            ) {
