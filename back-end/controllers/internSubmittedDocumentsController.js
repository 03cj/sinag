  ...
  try {
    ...
    const interns = await Intern.findAll({
      ...
      include: [
        ...
        { model: InternDocuments, required: false },
        { model: Company, required: false }, // ✅ ADD THIS
      ],
    ...
    interns.forEach((intern, index) => {
      ...
      doc.text(mark(docs.notarized_agreement), cols.nc.x, y + 5, {
        width: cols.nc.w,
        align: 'center',
      });

      doc.text(mark(docs.medical_cert), cols.med.x, y + 5, {
        width: cols.med.w,
        align: 'center',
      });

      doc.text(mark(docs.insurance), cols.ins.x, y + 5, {
        width: cols.ins.w,
        align: 'center',
      });

      // ✅ MOA COMES FROM COMPANY
      const moaFile = intern.Company?.moaFile;
      doc.text(mark(moaFile), cols.moa.x, y + 5, {
        width: cols.moa.w,
        align: 'center',
      });

      doc.text(mark(docs.cor), cols.cor.x, y + 5, {
        width: cols.cor.w,
        align: 'center',
      });

      doc.text(mark(docs.consent_form), cols.ia.x, y + 5, {
        width: cols.ia.w,
        align: 'center',
      });

      doc.text(mark(docs.resume), cols.res.x, y + 5, {
        width: cols.res.w,
        align: 'center',
      });
